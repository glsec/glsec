...
```sh
gh attestation verify glsec_1.18.0_linux_amd64.tar.gz --owner glsec
```
...
```sh
cosign verify ghcr.io/glsec/glsec:1.18.0 \
...
```sh
curl -sSLO https://github.com/glsec/glsec/releases/download/v1.18.0/checksums.txt
...
glsec:
  stage: test
  image:
    name: ghcr.io/glsec/glsec:1.18.0
    entrypoint: [""]
  script:
    - glsec .gitlab-ci.yml
```

The `entrypoint: [""]` override is required: the image sets `ENTRYPOINT ["glsec"]` for `docker run` convenience, which conflicts with GitLab Runner's shell wrapper. Pin to a specific tag (`1.18.0`, not `:latest`) for reproducible pipelines.
