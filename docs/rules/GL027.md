...
glsec flags variables defined in long-form (`value:` / `description:` mapping) whose name ends with a secret-like suffix (`_TOKEN`, `_SECRET`, `_PASSWORD`, `_PASSWD`, `_PASS`, `_PWD`, `_KEY`, `_CREDENTIAL`, `_CERT`) but lack `masked: true`. Without masking, GitLab echoes the variable's value to the job log whenever it expands the variable, making it readable by anyone with log access.
